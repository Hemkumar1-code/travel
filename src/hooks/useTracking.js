...
export function useTracking(userId, userName) {
  ...
  const handleNewLocation = (position) => {
    ...
    // Build path using ref for background-safe access
    pathRef.current = (() => {
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      // Start watchPosition
      // On Android with ACCESS_BACKGROUND_LOCATION permission in AndroidManifest,
      // this continues even when screen is off
      const watchId = await Geolocation.watchPosition(
        {
          enableHighAccuracy: true,
          timeout: 15000,
          maximumAge: 0, // always fresh — no cache when screen off
        },
        (position, err) => {
          if (err) {
      ...
      try {
        ...
        if (pos) handleNewLocation(pos);
      } catch (_) {}

    } catch (err) {
      setError(err.message || 'Critical error starting tracking');
      setIsTracking(false);
    }
  };
  ...
  const stopTracking = async () => {
    ...
    setTripData(summary);
    try {
      if (currentPath.length > 0) await push(ref(db, 'trips'), summary);
